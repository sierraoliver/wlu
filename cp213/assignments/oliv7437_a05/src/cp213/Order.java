...
public class Order implements Printable {
	...
	for (Entry<MenuItem, Integer> entry : map.entrySet()) {
	    BigDecimal value = new BigDecimal(entry.getValue().floatValue());
	    total = total.add(entry.getKey().getPrice().multiply(value));
	}
	...
	for (Map.Entry<MenuItem, Integer> entry : map.entrySet()) {
	    ...
	    itemList += String.format(lineFormat, entry.getKey().getListing(), entry.getValue(),
		    entry.getKey().getPrice().floatValue(),
		    (entry.getKey().getPrice().floatValue() * entry.getValue()));

	}
	itemList += "\n";
	itemList += String.format(totalFormat, "Subtotal", this.getSubTotal().floatValue());
	itemList += String.format(totalFormat, "Taxes", this.getTaxes().floatValue());
	itemList += String.format(totalFormat, "Total", this.getTotal().floatValue());
