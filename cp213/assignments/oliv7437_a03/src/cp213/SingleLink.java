...
public abstract class SingleLink<T> implements Iterable<T> {
	...
	if (this.length == 0) {
	    ...
	    SingleNode<T> currentNode = this.rear;

	    if (currentNode == this.front) {
		this.front.setNext(source.front);
		this.rear = source.rear;
	...
	if (this.front == this.rear) {
	    sourceFront.setNext(this.rear);
	    this.front = sourceFront;
	} else {
	    SingleNode<T> currentFront = this.front;
	    sourceFront.setNext(currentFront);
	    this.front = sourceFront;
	}
	...
	SingleNode<T> sourceRear = source.rear;

	if (this.front == this.rear) {
	    this.front.setNext(sourceRear);
	    this.rear = sourceRear;
