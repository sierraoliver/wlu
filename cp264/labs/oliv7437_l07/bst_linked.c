...
static data_ptr bst_min_aux (bst_node *node){
    if (node -> left != NULL){
        bst_min_aux (node->left);
    }
    else {
        return node->item;
    }
...
static data_ptr bst_max_aux (bst_node *node){
    if (node -> right != NULL){
        bst_max_aux (node->right);
    }
    else {
        return node->item;
    }
...
static int bst_leaf_count_aux (bst_node *node){
    if (node == NULL){
        return 0;
    }
    ...
    return bst_leaf_count_aux (node->right) + bst_leaf_count_aux (node->left);
}

static int bst_inorder_aux (bst_node *node, data_ptr *items, int num){
    if (node != NULL){

        num = bst_inorder_aux(node->left, items, num);

        *(items+num)= node->item;
        num+=1;
        num = bst_inorder_aux(node->right, items,num);
...
static bool bst_balanced_aux (bst_node *node){
    if (node == NULL){
        return true;
    }
    int lheight = 0;
...
static void bst_free_aux (bst_node *node){
    if (node != NULL){
        if (node->left) bst_free_aux(node->left);
        if (node->right) bst_free_aux(node->right);
        free (node);
    }
    return;
}
...
bool bst_retrieve(bst_linked *source, const data_ptr key, data_ptr item) {
    ...
    while (node != NULL){
        int compare = data_compare (node->item, key);
        if (compare>0){
            node = node->left;
        }
        else if (compare<0){
...
bool bst_max(const bst_linked *source, data_ptr item) {
    ...
    if (source -> count>0){
       data_ptr max = bst_max_aux (source->root); 
       data_copy (max, item);
       return true;
    }
    else{
...
int bst_leaf_count(const bst_linked *source) {
    ...
    int number = bst_leaf_count_aux (source->root);
...
bool bst_balanced(const bst_linked *source) {
    ...
    bool balanced = bst_balanced_aux(source->root);
